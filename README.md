...
SillyTavern 默认在每次发送消息后，将**完整的聊天记录**上传到服务端并覆写文件。对于大型聊天（几十 MB、数百条消息），这意味着每发一条消息都要传输全部数据，导致保存极其缓慢。

本补丁将保存行为改为**增量追加**：只上传新增的消息，从秒级延迟降低到毫秒级。
...
前端序列化整个 chat 数组（header + 全部消息）
  ↓
POST /api/chats/save  →  上传 75MB JSON
  ↓
服务端覆写 .jsonl 文件
```

一个 786 条消息 / 75MB 的聊天，每次保存都要传输 75MB。实际上新增的只有几 KB。
...
用户发送一条消息
  ↓
前端检测：只是追加了新消息？旧消息没改？
  ├─ 是 → POST /api/chats/save-append  →  只上传几 KB 的新消息
  └─ 否 → POST /api/chats/save          →  回退到全量保存（编辑/删除/swipe 时）
```

### 核心机制

- **前端变更检测**：跟踪 `lastSavedChatLength` 和消息 hash，判断是否只有末尾追加
...
- SillyTavern **1.16.0**（`ghcr.io/sillytavern/sillytavern:latest`，截至 2025-03）
...
```bash
git clone https://github.com/YOUR_USERNAME/sillytavern-incremental-save.git
...
2. 再发一条消息 → 应看到 `/api/chats/save-append`（增量，请求 body 仅几 KB）
